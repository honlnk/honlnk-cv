...
isStream$1.duplex = stream => isStream$1.writable(stream) && isStream$1.readable(stream)
...
function fixPath() {
  ...
  process$2.env.PATH =
    shellPathSync() ||
    ['./node_modules/.bin', '/.nodebrew/current/bin', '/usr/local/bin', process$2.env.PATH].join(
      ':'
    )
}

const IMAGE_EXT_LIST = ['.png', '.jpg', '.jpeg', '.bmp', '.gif', '.svg', '.tiff', '.webp', '.avif']
...
class FileTypeParser {
  ...
  async parse(tokenizer) {
    ...
    if (
      this.check([
        0x62, 0x6f, 0x6f, 0x6b, 0x00, 0x00, 0x00, 0x00, 0x6d, 0x61, 0x72, 0x6b, 0x00, 0x00, 0x00,
        0x00,
      ])
    ) {
      return {
    ...
    if (this.check([0xff, 0xfe])) {
      ...
      if (
        this.check(
          [
            0xff, 0x0e, 0x53, 0x00, 0x6b, 0x00, 0x65, 0x00, 0x74, 0x00, 0x63, 0x00, 0x68, 0x00,
            0x55, 0x00, 0x70, 0x00, 0x20, 0x00, 0x4d, 0x00, 0x6f, 0x00, 0x64, 0x00, 0x65, 0x00,
            0x6c, 0x00,
          ],
          { offset: 2 }
        )
...
class PicGoCoreUploader {
  ...
  async uploadFiles(fileList) {
    ...
    let command = `${cli} upload ${fileList.map(item => `"${item}"`).join(' ')}`
...
class SettingTab extends obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle(toggle =>
        toggle.setValue(this.plugin.settings.uploadByClipSwitch).onChange(async value => {
    ...
    if (this.plugin.settings.uploader === 'PicGo-Core') {
      ...
      if (os !== 'Windows') {
        new obsidian.Setting(containerEl)
          ...
          .addToggle(toggle =>
            toggle.setValue(this.plugin.settings.fixPath).onChange(async value => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle(toggle =>
        toggle.setValue(this.plugin.settings.workOnNetWork).onChange(async value => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addTextArea(textArea =>
        textArea.setValue(this.plugin.settings.newWorkBlackDomains).onChange(async value => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle(toggle =>
        toggle.setValue(this.plugin.settings.applyImage).onChange(async value => {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle(toggle =>
        toggle.setValue(this.plugin.settings.deleteSource).onChange(async value => {
...
class imageAutoUploadPlugin extends obsidian.Plugin {
      ...
      if (encodedUri.startsWith('http')) {
        ...
        if (
          ...
          const filePath = pathBrowserify.resolve(
            pathBrowserify.join(basePath, pathBrowserify.dirname(activeFile.path)),
  ...
  setupPasteHandler() {
      ...
      this.app.workspace.on('editor-paste', (evt, editor, markdownView) => {
        ...
        if (this.settings.workOnNetWork) {
          ...
          const imageList = this.helper
            ...
            .filter(
              image => !this.helper.hasBlackDomain(image.path, this.settings.newWorkBlackDomains)
            )
          if (imageList.length !== 0) {
            this.uploader.uploadFiles(imageList.map(item => item.path)).then(res => {
    ...
    this.registerEvent(
      this.app.workspace.on('editor-drop', async (evt, editor, markdownView) => {
