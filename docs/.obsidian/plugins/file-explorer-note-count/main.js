...
const countFolderChildren = (folder, filter) => {
  ...
  for (const af of folder.children) {
    if (filter(af)) count++
    if (af instanceof obsidian.TFolder) count += countFolderChildren(af, filter)
  }
  return count
}
...
const setupRootCount = plugin => {
  if (plugin.rootFolderEl) {
    ...
    if (rootFolderElChildren && rootFolderElChildren.length > 0) {
      let totalCount = countFolderChildren(
...
class FileExplorerNoteCount extends obsidian.Plugin {
  constructor() {
    ...
    this.explorerNavHeaderSelector =
    ...
    this.initialize = (revert = false) => {
      ...
      let explorerHeaderEl = document.querySelector(
        `${this.explorerNavHeaderSelector} .${this.rootFolderClassName}`
      )
    ...
    this.setupRootFolder = (revert = false) => {
      ...
      const root =
        (_c =
          (_b = (_a = this.fileExplorer) === null || _a === void 0 ? void 0 : _a.fileItems) ===
            null || _b === void 0
            ? void 0
            : _b['/']) !== null && _c !== void 0
          ? _c
