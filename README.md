# 鸿影 | Java全栈开发工程师

## 基本信息

<!--
基本信息部分支持的字段：
个人信息: 姓名、年龄、性别、生日
职业信息: 职位、工作经验、学历、专业
联系方式: 手机、邮箱、微信、QQ
位置信息: 期望城市、期望薪资
在线平台: 网站、GitHub、Gitee、技术博客、LinkedIn
其他信息: 求职状态、其他

格式：- **字段名**: 内容
示例：- **姓名**: 张三
-->

- **姓名**: 鸿影（网名）
- **年龄**: 21岁
- **职位**: Java全栈开发工程师
- **期望薪资**: 5-8K
- **学历**: 专科
- **期望城市**: 济南
- **工作经验**: 1年经验
- **求职状态**: 在职-考虑机会
- **在线简历**: https://cv.honlnk.top
- **手机**: 19563491369
- **邮箱**: jhshonlnk@gmail.com
- **网站**: https://www.honlnk.top
- **Gitee**: https://gitee.com/hong-ying-19
- **GitHub**: https://github.com/honlnk

## 核心优势

### 全栈开发

- SpringBoot+Vue.js技术栈
- MySQL+Redis数据层
- 独立完成2个全栈项目开发

### 技术深度

- SpringBoot3/MybatisPlus/SpringAI
- Vue3+Pinia+ElementPlus
- Hutool工具链

### 工程能力

- OCR/AI集成（百度智能云/通义百练）
- 前后端部署经验
- 跨端协同开发能力

## 项目经历

### 词砖·智能单词系统

**角色**: 全栈开发 | **时间**: 2024.11-至今

- 集成牛津/百度翻译API实现多维度单词解析，开发AI辅助学习场景（SpringAI+通义百练）
- 创新应用IndexedDB实现浏览器端数据缓存，响应速度拥有显著提升

**技术栈**: Vue3 | ElementPlus | SpringBoot3 | Mybatis | Redis

### 逐雁排程系统

**角色**: 全栈开发 | **时间**: 2024.02-2024.05

- 基于百度OCR实现表格智能识别，自主设计值班表排表算法，值班表生成效率提升90%
- 设计密钥认证体系替代传统登录，提升用户留存率

**技术栈**: Vue3 | SpringBoot | Mybatis | Hutool

### 高考志愿系统

**角色**: 独立前端 | **时间**: 2023.05-2023.11

- 首创霍兰德测试与志愿推荐融合算法
- 获省级二等奖/国家三等奖

**技术栈**: Vue2 | Vuex | Axios

## 工作经历

### 鸿影科技有限公司
**职位**: 前端开发工程师 | **时间**: 2024.01-2024.06

- 负责公司核心产品的前端开发，使用Vue3+TypeScript技术栈
- 参与需求分析和技术方案设计，与后端协作完成API对接
- **主要成就**: 重构首页架构，首屏加载时间减少65%，Lighthouse评分提升至95分
- **主要成就**: 封装通用组件库，包含25+组件，开发效率提升45%
- **主要成就**: 建立前端监控体系，错误捕获率提升90%，问题定位时间减少70%

### 创新网络技术公司
**职位**: Vue.js开发实习生 | **时间**: 2023.07-2023.12

- 协助开发和维护公司管理后台系统，参与功能模块设计与实现
- 使用Vue3+Pinia进行状态管理，ElementPlus构建用户界面
- 修复和优化现有代码，提升系统性能和用户体验
- **主要成就**: 完成用户权限模块开发，支持10种角色和50+权限控制
- **主要成就**: 优化表格组件性能，万条数据渲染时间从8秒缩短至1.5秒

## 教育背景

**学校**: 山东商业职业技术学院 | **专业**: 软件技术（专科）| **时间**: 2022-2025

### 校园经历

- 自主学习15+项技术栈，独立开发2个项目，共落地4个项目
- 获得技能类比赛，国奖1项，省奖多项
- 担任云计算实训中心学生负责人，时长一年半，与部门师生合作管理学院的实训室
- 与其他同学合作开发"实训室管理平台"
- 积极与同学老师交流学习先进技术栈

## 附加价值

### 🚀 技术视野

- 持续跟进SpringAI/大模型应用开发，落地2个AI集成项目
- 对通用大模型使用与训练有较高的兴趣

### 🛡️ 质量保障

- 开发"实训室管理平台"实现全年0教学事故
- 开发"逐雁排程"提升排表效率达90%

### 📚 学习能力

- 自主学习15+技术栈，2年内从零基础到全栈开发者
